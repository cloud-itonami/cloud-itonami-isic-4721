(ns foodretailops.phase
  "Phase 0->3 staged rollout for the ISIC-4721 specialized food retail
  ...
  needs a human to actually look at it -- this op never itself finalizes
  a food-safety-clearance decision. `foodretailops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently -- two
  layers, not one, agree on this."
  (:require [foodretailops.governor :as governor]))
...
(defn verdict->disposition
  "Map a FoodRetailGovernor verdict to a base disposition before the
  phase gate."
  [verdict]
