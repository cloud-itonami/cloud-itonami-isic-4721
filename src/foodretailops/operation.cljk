(ns foodretailops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (FoodRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the FoodRetailGovernor (:govern) and
  ...
  human food-retail operations coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [foodretailops.advisor :as advisor]
            [foodretailops.governor :as governor]
            [foodretailops.phase :as phase]
...
(defn build
  ...
  `foodretailops.store/Store`).
  opts:
    :advisor      -- a `foodretailops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FoodRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; FoodRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
