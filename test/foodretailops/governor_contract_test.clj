(ns foodretailops.governor-contract-test
  ...
  (testing "clean sales-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-sales-record :store-id "store-1" :patch {:units-sold 42 :item "ground beef"}}
                               ctx)]
      (is (some? result))
  ...
  (testing ":flag-food-safety-concern escalates for human approval, regardless of phase/confidence"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-food-safety-concern :store-id "store-1"
                                :patch {:concern "spoilage suspected" :confidence 0.99}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :store-id "store-1"
                                :patch {:item "walk-in cooler repair" :quantity 1 :estimated-cost 2400.0}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
